...
const DashbaordCard = ({ title, value, iconBackground = "#27d095", icon }) => {
  return (
    <Paper sx={{ backgroundColor: "#fff", borderRadius: "8px", p: 2, mt: 2, color: "#070707" }}>
      <Stack direction={"row"} alignItems={"center"} sx={{ gap: 1 }}>
        ...
        <Box sx={{ display: "flex", flexDirection: "column", gap: 0.5, justifyContent: "center" }}>
          <Typography variant="h5" sx={{ fontSize: "20px", fontWeight: "400", }}>{title}</Typography>
          <Typography variant="h6" sx={{ fontSize: "16px", fontWeight: "400",  }}>{value}</Typography>
        </Box>
      </Stack>
    </Paper>
  )
